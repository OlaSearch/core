...
var NoResults = function NoResults(_ref) {
  ...
  if (totalResults === 0 && suggestedTerm && facets.length > 0) {
    message = _react2['default'].createElement(
      'div',
      null,
      translate('no_results_found_filters_too_restrictive', { q: q }, true, { tagName: 'span' }),
      _react2['default'].createElement(
        'button',
        { className: 'ola-reset-filters', type: 'button', onClick: removeFilters },
        'Remove filters'
      )
    );
  } else {
    if (!q) {
      message = _react2['default'].createElement(
        'div',
        null,
        translate('no_results_found_filters_only', null, true, { tagName: 'span' }),
        _react2['default'].createElement(
          'button',
          { className: 'ola-reset-filters', type: 'button', onClick: removeFilters },
          'Remove filters'
