  ...
  function AutoComplete(props) {
    ...
    _this.onChange = function (term, searchInput) {
      ...
      if (allowedCharacters && !(0, _utilities.checkForAllowedCharacters)(term, allowedCharacters)) {
        ...
        _this.props.executeFuzzyAutoSuggest(term).then(function (response, xhr) {
          var results = (0, _utilities.pickDeep)(response.suggest, 'suggestions');
          /* Parse payload */
          var res = [];
          ...
          for (var i = 0; i < results.length; i++) {
            ...
            if (payload.categories && payload.categories.length && !categoryFound) {
              ...
              var categories = _this.props.visibleCategoryGroups ? payload.categories.filter(function (item) {
              ...
              for (var j = 0; j < totalCategories; j++) {
                var _payload$categories$j = payload.categories[j].split('|');

                var _payload$categories$j2 = require('../../../.babelhelper.js').slicedToArray(_payload$categories$j, 1);

                var name = _payload$categories$j2[0];

                var displayName = facet.facetNames[name] || name;
                res.push(require('../../../.babelhelper.js')['extends']({}, item, {
                  category_id: name,
              ...
              res.push(require('../../../.babelhelper.js')['extends']({}, item, { payload: payload }));
            }
          }
          _this.setState({
            results: res,
    ...
    _this.onSubmit = function (event) {
      ...
      /* Update query term */
      _this.props.updateQueryTerm(_this.state.q);
    ...
    _this.onFuzzySelect = function (suggestion) {
      ...
      var facet = void 0;
      var isTaxonomy = payload.taxo_group;
      var isCategory = suggestion.category_group;
      var rawSuggestion = payload.suggestion_raw;
      var term = rawSuggestion || suggestion.term;
       ...
       */
      if (isTaxonomy) {
        facet = (0, _find2['default'])((0, _propEq2['default'])('name', payload.taxo_group))(_this.context.config.facets);
        /* Remove all selected facets */
        _this.props.removeAllFacets();
        /* Add facet */
        _this.props.replaceFacet(facet, payload.taxo_id);
        /* Add query term */
        _this.props.updateQueryTerm('');
       ...
       */
      if (isCategory) {
        facet = (0, _find2['default'])((0, _propEq2['default'])('name', suggestion.category_group))(_this.context.config.facets);
        /* Remove all selected facets */
        _this.props.removeAllFacets();
        /* Replace facet */
        _this.props.replaceFacet(facet, suggestion.category_id);
        /* Add query term */
