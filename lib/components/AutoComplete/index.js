...
var AutoComplete = function (_React$Component) {
  ...
  function AutoComplete(props) {
    ...
    var _this = require('../../../.babelhelper.js').possibleConstructorReturn(this, (AutoComplete.__proto__ || Object.getPrototypeOf(AutoComplete)).call(this, props));
        ...
        _this.props.executeFuzzyAutoSuggest(term).then(function (results) {
          ...
          for (var i = 0, len = results.length; i < len; i++) {
            var _results$i = results[i],
                ...
                rest = require('../../../.babelhelper.js').objectWithoutProperties(_results$i, ['payload']);
            ...
            if (isCategory) {
              ...
              for (var j = 0; j < totalCategories; j++) {
                var _payload$taxo_terms$j = payload.taxo_terms[j].split('|'),
                    _payload$taxo_terms$j2 = require('../../../.babelhelper.js').slicedToArray(_payload$taxo_terms$j, 1),
                    name = _payload$taxo_terms$j2[0];

                var _ref = payload.taxo_paths ? payload.taxo_paths[j].split('|') : [],
                    _ref2 = require('../../../.babelhelper.js').slicedToArray(_ref, 1),
                    path = _ref2[0];
