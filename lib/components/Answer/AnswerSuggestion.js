...
var _equals = require('rambda/lib/equals');
...
function AnswerSuggestion(_ref) {
  var answer = _ref.answer,
      onChange = _ref.onChange,
      onSkipIntent = _ref.onSkipIntent;

  if (!answer.suggestions || !(0, _keys2['default'])(answer.suggestions).length) {
    return null;
  }
  ...
  keys.map(function (key) {
    ...
    /* Generate dropdown */
    text[start] = _react2['default'].createElement(_AnswerDropdown2['default'], {
      key: key,
      item: key,
      options: options,
      active: selection,
  ...
  keys.map(function (key) {
    var _suggestions$key$posi = suggestions[key].position,
