...
var Input = function (_React$Component) {
	...
	function Input(props) {
		...
		_this.onClear = function (event) {
			...
			if (event.target.nodeName == 'INPUT' && !event.target.value) {
				event.target.blur();
				_this.props.handleClickOutside.call(_this, event);
				return;
			}
		...
		_this.onKeyDown = function (event) {
			...
			switch (event.which) {
				...
				case 27:
					// Esc
					_this.onClear.call(_this, event);
					break;
				case 38:
					// Up
					onMove('up');
	...
	_createClass(Input, [{
		...
		value: function render() {
			...
			var button = q ? _react2['default'].createElement('button', { type: 'reset', className: 'ola-btn ola-clear-button', onClick: this.onClear }) : _react2['default'].createElement('button', { type: 'button', className: 'ola-btn ola-search-button', onClick: function onClick() {
