...
var CheckboxFilter = function (_React$Component) {
  ...
  function CheckboxFilter(props) {
    ...
    _this.itemRenderer = function (values, index) {
  ...
  CheckboxFilter.prototype.render = function render() {
    ...
    var values = facet.values,
        ...
        _facet$defaultValue = facet.defaultValue,
        defaultValue = _facet$defaultValue === undefined ? [] : _facet$defaultValue;
    ...
    if (fixedValues && fixedValues.length) {
       ...
       */
      var selectedValue = [].concat(defaultValue.map(function (item) {
        return item.replace(/(-)/gi, '');
      }), fixedValues);
      var acceptedValues = values.filter(function (_ref2) {
        var name = _ref2.name,
            count = _ref2.count;
        return selectedValue.indexOf(name) !== -1;
      });

      if (!acceptedValues.length && !showIfEmpty) return null;
      var totalValueCount = acceptedValues.reduce(function (acc, item) {
        acc += item.count;
        return acc;
      }, 0);
      values = fixedValues.map(function (item) {
        return {
          name: item,
          count: item === _Settings.ALL_VALUES ? totalValueCount : acceptedValues.filter(function (_ref3) {
            var name = _ref3.name;
            return name === item;
...
function CheckBoxItem(props) {
  ...
  return _react2['default'].createElement(
    ...
    _react2['default'].createElement('input', { type: 'checkbox', checked: isActive, onChange: onChecked }),
    _react2['default'].createElement(
      'span',
...
exports['default'] = (0, _withTranslate2['default'])((0, _withToggle2['default'])(CheckboxFilter));