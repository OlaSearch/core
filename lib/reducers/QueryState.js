...
exports['default'] = function () {
  ...
  switch (action.type) {
    ...
    case _ActionTypes2['default'].UPDATE_STATE_FROM_QUERY:
      return _extends({}, (0, _urlSync.parseQueryString)(state, action.config), {
        referrer: ''
      });
    ...
    case _ActionTypes2['default'].ADD_FACET:
      ...
      value = action.value;
      facet = action.facet;
      props = (0, _ramda.omit)('values', facet);
    ...
    case _ActionTypes2['default'].REMOVE_FACET:
      ...
      for (var i = fq.length - 1; i >= 0; i--) {
        ...
        if (cur.name === facet.name) {
          ...
          _selected.splice((0, _ramda.indexOf)(value, _selected), 1);

          if (!_selected.length) fq = [].concat(_toConsumableArray(fq.slice(0, i)), _toConsumableArray(fq.slice(i + 1)));
        }
      }
    ...
    case _ActionTypes2['default'].REPLACE_FACET:
      ...
      if (index === null) {
        fq = [].concat(_toConsumableArray(fq), [_extends({}, props, {
          selected: [value]
        })]);
      } else {
