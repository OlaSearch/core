...
function parseQueryString(initialState, config) {
  ...
  for (var p in qs) {
    ...
    /* prevent XSS */
    qs[p] = (0, _utilities.sanitizeText)(qs[p]);

    if (config.replaceQueryParamName && p === _Settings.QUERY_ALT_NAME) {
      qs['q'] = (0, _utilities.sanitizeText)(qs[p]);
    }
  ...
  if (facetQuery) {
    ...
    var fq = facetQuery.filter(function (item) {
      ...
      return (0, _values2['default'])(config.fieldMappings).indexOf(name) !== -1;
  ...
  return (0, _extends3['default'])({}, initialState, qs, {
    sort: sort /* Set sort */
  }, facetQueryObject, filtersObject, tokensObject);
}