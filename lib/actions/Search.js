...
exports.navigateToResultsPage = navigateToResultsPage;
...
function executeSearch(payload) {
  return function (dispatch, getState) {
    ...
    if (payload && payload.forceRedirect) {
      window.location.href = payload.searchPageUrl + '?' + (0, _urlSync.buildQueryString)(query);
      return;
    }
...
function removeFacet(facet, value) {
  var resetAllFacets = arguments.length <= 2 || arguments[2] === undefined ? false : arguments[2];
...
function initSearch(_ref3) {
  ...
  var _ref3$urlSync = _ref3.urlSync;
  var urlSync = _ref3$urlSync === undefined ? true : _ref3$urlSync;
