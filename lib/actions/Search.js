...
function executeSearch(payload) {
  return function (dispatch, getState) {
    ...
    if (allowedCharacters && !(0, _utilities.checkForAllowedCharacters)(q, allowedCharacters) || !(isSearchActive || !!q || facet_query.length || filters.length) /* If no query and search is not active (searchOnLoad = false) */
    ) {
        // Terminate search
