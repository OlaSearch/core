...
function log(params) {
  ...
  var eventType = params.eventType,
      ...
      state = params.state,
      rest = (0, _objectWithoutProperties3['default'])(params, ['eventType', 'debounce', 'state']);
  ...
  return function (dispatch, getState) {
    ...
    if (debounce) {
      return debounceLog((0, _extends3['default'])({ dispatch: dispatch, state: _state, eventType: eventType }, rest));
    }
    return submitLog((0, _extends3['default'])({ dispatch: dispatch, state: _state, eventType: eventType }, rest));
  };
}
...
function submitLog(args) {
  var dispatch = args.dispatch,
      state = args.state,
      setNewUser = args.setNewUser,
      rest = (0, _objectWithoutProperties3['default'])(args, ['dispatch', 'state', 'setNewUser']);
  ...
  if (state.Context.isNewUser) {
    ...
    _storage.cookies.set((0, _utilities.getKey)(_Settings.USER_NEW_KEY, state.AppState.namespace), false, _Settings.USER_SESSION_EXPIRY_DAYS);
    dispatch({
