...
module.exports = function (config, searchProvider) {
  ...
  if (config.logger && config.logger.enabled) {
    var userSession = _storage2['default'].cookies.get(_utilities2['default'].getKey(_Settings.USER_SESSION_KEY, config.namespace));
    if (!userSession) {
      _storage2['default'].cookies.set(_utilities2['default'].getKey(_Settings.USER_SESSION_KEY, config.namespace), _utilities2['default'].uuid(), _Settings.USER_SESSION_EXPIRY_DAYS);
    }
    store.dispatch({
      type: _ActionTypes2['default'].SET_USER_SESSION,
