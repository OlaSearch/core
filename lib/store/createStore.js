...
exports['default'] = function (config, searchProvider) {
  ...
  var connectionWatcher = (0, _onlineStatusEnhancer2['default'])();

  /* Store */
  var store;

  if (process.env.NODE_ENV === 'production') {
    store = (0, _redux.createStore)(olaReducers, _redux.compose.apply(undefined, [connectionWatcher].concat(enhancers, [_redux.applyMiddleware.apply(undefined, [_reduxThunk2['default'], olaMiddleWare, persistMiddleware].concat(middlewares))])));
  } else {
    ...
    store = (0, _redux.createStore)(olaReducers, _redux.compose.apply(undefined, [connectionWatcher].concat(enhancers, [_redux.applyMiddleware.apply(undefined, [_reduxThunk2['default'], olaMiddleWare, persistMiddleware, logger].concat(middlewares))])));
  }

  var _prepareStoreState = (0, _prepareStore.prepareStoreState)({
    config: config
  }),
      userSession = _prepareStoreState.userSession,
      searchSession = _prepareStoreState.searchSession,
      isNewUser = _prepareStoreState.isNewUser,
      rest = (0, _objectWithoutProperties3['default'])(_prepareStoreState, ['userSession', 'searchSession', 'isNewUser']);
...
var _onlineStatusEnhancer = require('./../enhancer/onlineStatusEnhancer');

var _onlineStatusEnhancer2 = _interopRequireDefault(_onlineStatusEnhancer);
