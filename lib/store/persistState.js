...
var statesToTrack = exports.statesToTrack = ['history', 'bookmarks', 'isSidebarOpen', 'queryIds', 'queriesById', 'view'];
...
function persistState(action, getState, namespace) {
  ...
  switch (action.type) {
    ...
    case _ActionTypes2['default'].SET_LOCALE:
      return _storage2['default'].cookies.set(_Settings.LOCALE_STORAGE_KEY, state.Intl.locale, _Settings.LOCALE_STORAGE_TTL, namespace);
    ...
    case _ActionTypes2['default'].CHANGE_VIEW:
      return _storage2['default'].set(_Settings.OLA_STORAGE_KEY, (0, _utilities.pick)(statesToTrack, state.AppState), namespace);
  }
}