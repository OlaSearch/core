...
function persistState(action, getState, namespace) {
  ...
  switch (action.type) {
    ...
    case _ActionTypes2['default'].ADD_CONTEXT_FIELD:
      return _storage2['default'].cookies.set(_Settings.CONTEXT_STORAGE_KEY, JSON.stringify(state.Context), _Settings.CONTEXT_STORAGE_TTL, namespace);
  }
}