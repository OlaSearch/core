...
var _uniqBy = require('ramda/src/uniqBy');
...
var utilities = {
    ...
    for (var i = 0, len = rules.length; i < len; i++) {
      ...
      if (Array.isArray(rule)) {
        ...
        for (var _field in rule) {
          ...
          if (!_fieldValue || _fieldValue && !_fieldValue.toString().match(new RegExp(rule[_field] + '$', 'gi'))) matched = false;
        }
      }
  ...
  sortHistory: function sortHistory(a, b) {
    var a1 = a.popularity;
    var b1 = b.popularity;
    var a2 = a.dateAdded;
    var b2 = b.dateAdded;
  ...
  sanitizeNumbers: function sanitizeNumbers(text) {
    if (typeof text !== 'string') return text;
    return parseFloat(text.replace(/<(?:.*|\n)*?>/gm, ''));
  },
  mergeResultsWithHistory: function mergeResultsWithHistory(history, results, query) {
    ...
    var uniqByTerm = function uniqByTerm(a) {
      return a.term;
    };
    ...
    /* Remove duplicates */
    history = (0, _uniqBy2['default'])(uniqByTerm, history);
