...
var utilities = {
  ...
  persistState: function persistState(action, getState, namespace) {
    switch (action.type) {
      case _ActionTypes2['default'].ADD_BOOKMARK:
      case _ActionTypes2['default'].REMOVE_BOOKMARK:
        _storage2['default'].set(_Settings.BOOKMARKS_STORAGE_KEY, getState().AppState.bookmarks, namespace);
        break;

      case _ActionTypes2['default'].ADD_HISTORY:
      case _ActionTypes2['default'].CLEAR_HISTORY:
        _storage2['default'].set(_Settings.HISTORY_STORAGE_KEY, getState().AppState.history, namespace);
        break;
    }
  }
};
