...
var dateRe = /(\d+)[-.\/](\d+)[-.\/](\d+)/;
var timeRe = /(\d+):(\d+)(?::(\d+))?(\.\d+)?(?:\s*(?:(a)|(p))\.?m\.?)?(\s*(?:Z|GMT|UTC)?(?:([-+]\d\d):?(\d\d)?)?)?/i;
var unescapeRe = /\\(.)/g;
var map = { D: 'Date', h: 'Hours', m: 'Minutes', s: 'Seconds', S: 'Milliseconds' };
var masks = {
  'default': 'DDD MMM DD YYYY hh:mm:ss',
  'iso': 'UTC:YYYY-MM-DD\'T\'hh:mm:ss\'Z\''
};
var names = 'JanFebMarAprMayJunJulAugSepOctNovDecJanuaryFebruaryMarchAprilMayJuneJulyAugustSeptemberOctoberNovemberDecemberSunMonTueWedThuFriSatSundayMondayTuesdayWednesdayThursdayFridaySaturday'.match(/.[a-z]+/g);

var DateParser = {
  format: function format(date, mask, _zone) {
    if (!date) return;
    ...
    var undef, zonediff;
    var origin = +date;
    var get = 'get' + (mask.slice(0, 4) === 'UTC:' ? (mask = mask.slice(4), 'UTC') : '');
    var zone = _zone === undef ? date._z : _zone;
  ...
  parse: function parse(inputDate, format, zoneOut, zoneIn) {
    var undef, match, year, month;
    var date = new Date();
    var num = +inputDate || '' + inputDate;

    if (isNaN(num)) {
      match = num.match(dateRe);
      if (match) {
      ...
      date.setHours(match[6] && match[1] < 12 ? +match[1] + 12 : match[5] && match[1] === 12 ? 0 : match[1], match[2], match[3] | 0, 1000 * match[4] | 0);
